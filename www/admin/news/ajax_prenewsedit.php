...
require "../inc/config_perm_ajax.php";//一级目录和二级文件的访问权限判断
  if($ajax_onuserperm_editprem=='1'){
   ...
   //取出fatherid信息
   $strSQL="SELECT fatherid,name,id_newsdir,intro FROM newsdir WHERE  id_newsdir=$newsid and  dele='1' order by ordernum ASC";
   $objDB->Execute($strSQL);
   $newsid=$objDB->fields();
   
   //取出一级目录
  $strSQL="SELECT id_newsdir,name FROM newsdir WHERE level='1' and dele='1' order by ordernum ASC";
  $objDB->Execute($strSQL);
...
for($i=0;$i<sizeof($arrnewdir1);$i++){
  ...
  if($arrnewdir1[$i][id_newsdir]==$newsid[fatherid]){
  $newsid1=$newsid1.'<option value='.$arrnewdir1[$i][id_newsdir].' selected>'.$arrnewdir1[$i][name].'</option>';
  }else{
  $newsid1=$newsid1.'<option value='.$arrnewdir1[$i][id_newsdir].'>'.$arrnewdir1[$i][name].'</option>';
  }
...
$newsid_ajaxpreedit=$newsid1.'</select>';//在AJAX弹窗生成一级目录下拉菜单
  ...
  $arr['selectid']=$newsid[id_newsdir];//当前所选菜单id
