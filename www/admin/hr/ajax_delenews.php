...
require "../inc/config_perm_ajax.php";//一级目录和二级文件的访问权限判断
 if($ajax_onuserperm_deleprem=='1'){
   ...
   if($isdel=='1'){//放入回收站
	$strSQL="update newsinfo set dele='0',deldate=now()  WHERE id_newsinfo='".$delnewsid."'";
   ...
   if($isdel=='0'){//永久删除
	$strSQL="DELETE FROM newsinfo WHERE id_newsinfo='".$delnewsid."'";
	$objDB->Execute($strSQL);
    $arr['info']="永久删除成功!";
    $myjson= json_encode($arr);
    echo $myjson;
   ...
   if($isdel=='3'){//撤销删除
	$strSQL="update newsinfo set dele='1',deldate=now()  WHERE id_newsinfo='".$delnewsid."'";
	$objDB->Execute($strSQL);
    $arr['info']="撤销删除成功!";
    $myjson= json_encode($arr);
    echo $myjson;
